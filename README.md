...
- 世界推衍设置：读取 `Codex 世界引擎` 或旧世界引擎快照，并导出推衍强度、时间策略、时间尺度、地点策略、最大更新数、后台事件和因果约束。
- 资料补全设置：导出搜索范围、候选查询、天气、全球要闻、当地要闻、日历、文化、经济、交通、来源上限和不同资料的刷新频率。
- 任务快照会附带从最近楼层抽取的时间地点线索，供 Codex 工作流判断当前时间点和地点。
- 使用右下角悬浮按钮打开或关闭独立控制台，面板可拖动并持久化位置。
- 在 SillyTavern 扩展设置页保留轻量入口和快捷操作，主 UI 不依赖扩展抽屉。
